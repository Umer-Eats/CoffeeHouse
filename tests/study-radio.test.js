...
function boot(saved){
  ...
  return {player,events,tick,controls,stationButtons,credit,choices,stored:()=>stored,loaded:()=>loaded,cued:()=>cued,frame};
}
...
test('genre buttons switch playlists - hiphop',()=>{
  const app=boot(saved(false));
...
test('genre buttons switch playlists - jazz',()=>{
  const app=boot(saved(false));
  app.stationButtons[0].click();
  assert.ok(app.frame.src.includes('PL9ndRPYDuLTe4zuQo8B3kfignCJ1RYyT7'));
  assert.equal(app.credit.textContent,'@ICYFOG');
});
