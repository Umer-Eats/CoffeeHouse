...
test('desktop alerts request permission only on explicit enable and provide native fallback sound',async()=>{
 ...
 assert.equal(Notification.permission,'granted');assert.equal(calls[1].options.silent,false);
...
test('supported browsers deliver through service worker persistent notifications',async()=>{
 ...
 assert.equal(calls[0],'/notification-worker.js');assert.equal(calls[1].title,'CoffeeHouse · Jamie');assert.equal(calls[1].options.data.channel,'dm:2');assert.equal(calls[1].options.silent,false);
